...
%               - If plot=1, a plot with the monitoring of the classification is shown on
%                 the screen (using scatter function). The plot can be:
...
%                       to alpha(1), second column refers to alpha(2), ...,
%                       last column refers to alpha(end).
...
%                       with j=1, 2, ...,  length(alpha), the 3D array  of
%                       size v-by-v-k containing the (constrained)
...
%
%
% This iterative algorithm extends to tclust the so called monitoring
% approach. The phylosophy is investigate how the results change as the
...
%{
    ...
    out=tclusteda(Y(:,1:2),3,[0.2 0.15 0.10 0.05 0],1,'refsteps',20,'plots',1)
    cascade
%}
...
if plots==1
    ...
    for j=1:lalpha
        ...
        if v>=2
            ...
            if alpha(j)~=0
                histFS(Y,10,IDX(:,j),[],[],colord)
            else
                histFS(Y,10,IDX(:,j),[],[],colord(2:k+1))
            end
        end
