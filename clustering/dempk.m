...
%               Data Types - single | double
% 
% hierOpt   :   Empty structure (default) or structure containing optional
%               input parameters for clusterdata built-in function.
%               See clusterdata and linkage function.
%               Example - 'hierOpt.linkage', 'ward' 
%                   [[[? Is it appropiate using the Ward linkage? ?]]]
% 
...
%               Example - 'Ysave',1 
%               Data Types - double
%                   [[ should it work for any number different from 0? ]]
% 
% Output:
%
...
%{
    ...
    hierOpt = struct;
    hierOpt.maxclust = g;
    hierOpt.linkage = 'weighted';
    hierOpt.distance = 'mahalanobis';

    DEMP = dempk(X, k, g, 'hierOpt', hierOpt, 'plots', 'ellipse');
...
%{
    ...
    % using dempk with hirearchical merging and alpha equal to noise level
...
options = struct('alpha', 0, 'plots', 0, 'tkmeansOut', 0, ...
    'tkmeansOpt', struct(), 'hierOpt', struct(), 'Ysave', 0); 
...
if nargin > 3
    ...
    if ~isstruct(options.hierOpt)
         error('FSDA:clusterdataCall','hierOpt must be a structure')
    end
end
...
if g >= 1 && mod(g, 1)==0 && g<=k
    ...
    if isempty(fieldnames(hierOpt)) 
        % if no additional options are specified in hierOpt structure 
        Hier = clusterdata(candVec, g); 
        
    else
        % add optional arguments to clusterdata
        hierOptNames = fieldnames(hierOpt);
        hierOptCell = cell(1,2*length(hierOptNames)); 
        for i = 1:numel(hierOptNames) 
            first =(i-1)*2+1;
            hierOptCell{first}=hierOptNames{i};
            hierOptCell{first+1}=extractfield(hierOpt, hierOptNames{i});
        end
            Hier = clusterdata(candVec, 'maxclust', g, hierOptCell{:}); 
    end
