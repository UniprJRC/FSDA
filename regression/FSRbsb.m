...
%The plot, which moniotrs the units belonging to subset in each step of the
%forward search shows that apart from unit 11 which enters the search in step m=78 all the other contaminated units enter the search in the last 19 steps.

%if we consider the seed state=500, we obtain a plot showing that the 20 contaminated units enter the search in the final 20 steps.
state=500;
randn('state', state);
X=randn(n,3);
y=3*randn(n,1)+X*bet;
