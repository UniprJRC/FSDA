function Wpdf = mWNChygepdf(x, m, w, accuracy)
%mWNChygepdf returns Wallenius' multivariate non-central hypergeometric probability density function
%
%<a href="matlab: docsearchFS('mWNChygepdf')">Link to the help function</a>
%
% This function calls function  CMultiWalleniusNCHypergeometricpdf which is a
...
% CMultiWalleniusNCHypergeometricpdf is the original one of Fog.
%
...
%  Output:
%
%           Wpdf : Wallenius' multivariate pdf values. 
...
%{
    % In presence of just two colors in the urn it is possible to use  WNChygepdf or mWNChygepdf.
    ...
    XX=[x n-x];
    m=[K M-K]; w=[100 1];
    wpdfCHK=mWNChygepdf(XX,m,w);
    assert(isequal(wpdf,wpdfCHK),'Call to WNChygepdf produces different results from call to mWNChygepdf')
%}
...
for i=1:size(x,1)
    Wpdf(i) = aux.CMultiWalleniusNCHypergeometricpdf(x(i,:),m,w,accuracy);
end
