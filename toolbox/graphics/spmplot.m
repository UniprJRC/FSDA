...
%{
    ...
    % labels determined automatically
    spmplot(meas);
    
    % labels are formatted
    plo=struct;
    plo.TickLabels = '%.2f';
    figure; spmplot(meas,'plo',plo);
...
%{
    %% Call of spmplot with option plo: long Labels are shorten.
    load head;
...
if isstruct(plo)
    ...
    if d>0
        TickLabelsFormat = plo.TickLabels;
    else
        TickLabelsFormat = 'auto';
    end

    d=find(strcmp('nameYlength',fplo));
    if d>0
        nameYlength = plo.nameYlength;
    else
        nameYlength = 0;
    end
...
elseif ischar(TickLabelsFormat) && strcmp(TickLabelsFormat(1),'%')
    ...
    for i=1:p
        format_ticklbl(AX(p,i),'X',TickLabelsFormat);
        format_ticklbl(AX(i,1),'Y',TickLabelsFormat);
    end
elseif ischar(TickLabelsFormat) && strcmp(TickLabelsFormat,'auto')
...
if nameYlength>0
    ...
    % do nothing: keep the Labels generated by gplotmatrix
end
...
%% Modifications of gplotmatrix start here
...
if colorBackground==true
    ...
    % hc.Limits=[-1 1]; % this produces a wrong colorbar
end
            ...
            if i~=j
                ...
                if method~="scatter"
                    ...
                    if i==1 && j==p
                        ...
                        delete(H(i,j,:)); % Delete data (this should accelerate the graphic generation)
                        cla(AX(i,j));
                    end
...
% function to adjust the lenght of a set of axis label
    function LabelStringCut = format_lbl_all(AX,axName,maxcar)
        %if nargin<3 || isempty(format)
        %    format = '%.1f';
        %end
        pp = size(AX,2);
        ...
        for ih=1:pp
            ...
            LabelStringCut{ih} = LabelStr(1:maxcar);
        end
        LabelStringCut = genvarname(LabelStringCut);
        set(hLabels,{'String'},LabelStringCut);
    end
