function [out] = FSRedaTobit(y,X,bsb,varargin)
%FSRedaTobit enables to monitor several quantities in each step of the forward search
%
%<a href="matlab: docsearchFS('FSRedaTobit')">Link to the help function</a>
%
...
%
%
% See also LXS.m, FSReda.m, FSRfanTobit
%
% References:
%
...
%{
    ...
    % Example of use of FSRedaTobit based on a starting point coming
    % from LMS.
    n=200;
    p=3;
    randn('state', 123456);
...
if nargin<2
    error('FSDA:FSRedaTobit:missingInputs','not enough input args')
end
...
if ~isempty(UserOptions)
    ...
    if length(varargin) ~= 2*length(UserOptions)
        error('FSDA:FSRedaTobit:WrongInputOpt','Number of supplied options is invalid. Probably values for some parameters are missing.');
    end
...
if bsb==0
    ...
    if nwhile==100
        warning('FSDA:FSRedaTobit:NoFullRank','Unable to randomly sample full rank matrix');
    end
    yb=y(bsb);
else
    Xb=X(bsb,:);
