...
% Find the cells.
cellLocations = matlab.codeanalyzer.internal.analyzeCode(code);
codeSections = cellLocations.CodeSections;
% codeSections can either be empty or have table representing the section properties.
if isempty(codeSections)
    % No code sections, add an implicit outer cell.
    cellStarts = 1;
    cellEnds = sum(code == newLine);
elseif codeSections.startLines(1) ~= 1
    % Input file has a function, add an implicit outer cell.
    cellStarts = [1; codeSections.startLines];
    cellEnds = [sum(code == newLine); codeSections.endLines];
else
    % Get the line boundaries for each cell in the file.
    cellStarts = codeSections.startLines;
    cellEnds = codeSections.endLines;
end
...
dom = matlab.io.xml.dom.Document('mscript');
...
for n = 1:length(chunkList)
    frag = matlab.internal.publishparser.getDOMFragment( ...
    dom, ...
