function [dom,laste] = evalmxdomNEW(f,dom,cellBoundaries,imagePrefix,imageDir,outputDir,options)
...
% Provide an empty, undocked figure.
if options.useNewFigure
    figureName = 'Figure';
    myFigure = figure( ...
        'IntegerHandle','off','NumberTitle','off','Name',figureName,'WindowStyle','normal');
    drawnow;
else
    myFigure = [];
end
...
data.pictureSizeList = cell(size(data.pictureList));
...
if ~isempty(laste)
    if options.catchError
        if options.displayError
            ...
            fprintf(2, '%s', formatError(laste,true));
            beep
        end
    else
        rethrow(laste)
...
% Get the current breakpoint status so we can restore it later. This may
% error when the file isn't parseable, so catch that as well.
try
    origHotlinks = feature('hotlinks',0);
...
% Initialize onCleanup function to remove the dynamic property MW_PublishGeneratedFigure
% and enable axes toolbars for a figure
resetAxesToolbarObj = onCleanup(@()restoreAxesToolbar());
...
delete(resetEchoObj);
delete(resetAxesToolbarObj);
end

function restoreAxesToolbar()
    figures = allchild(0);
    for i = 1:numel(figures)
        if isprop(figures(i), internal.matlab.publish.PublishFigures.publishGeneratedFigure)
            delete(findprop(figures(i), internal.matlab.publish.PublishFigures.publishGeneratedFigure));
        end
    end
end
...
for i = 1:numel(data.pictureList)
    ...
    sizeData = data.pictureSizeList{i};
    if ~isempty(sizeData)
        imgNode.setAttribute('width',sprintf('%ipx',sizeData(1)));
        imgNode.setAttribute('height',sprintf('%ipx',sizeData(2)));
    end
...
% Substitute non-printable characters with space. Characters excluded are 8,9,10 and 13.
text = regexprep(text, '[\x{0}-\x{7}\x{B}-\x{C}\x{E}-\x{1F}]', ' ');
...
for iPictureList = numel(data.pictureList):-1:1
    ...
    if any(strcmp(ext,{'.png','.jpg','.tif','.gif','.bmp'}))
        ...
        internal.matlab.publish.resizeIfNecessary( ...
            [data.baseImageName '.png'],'png', ...
            options.maxThumbnailWidth,options.maxThumbnailHeight,myFrame);
        break
    end
end
...
function s = getCauseString
    s = strip(getString(message('MATLAB:MException:CausedBy')));
end
