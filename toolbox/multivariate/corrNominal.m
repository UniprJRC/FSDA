...
%                 If N is a data matrix (supplied as a n-by-2 cell array
...
%               Character.
%               Character that identifies the method to use to compute the
...
%               degrees of freedom. 'fisher' uses the Fisher
...
%
%  dispresults : displays results on the screen. Boolean.
...
%    out.Chi2pval    =   scalar containing p-value of the $\chi^2$ index.
...
%      out.CramerV   =   1 x 4 vector that contains Cramer's V index,
...
%                        this index is specified is input option conflimMethodCramerV.
...
%                        \]
%    out.GKlambdayx  =   1 x 4 vector that contains index  $\lambda_{y|x}$
...
%                       \]
%                       If we take as a measure of variation $V(y)$ the Gini coefficient
%                       \[
...
%{
    % Example that starts from the original data matrix.
    N=[26 26 23 18 9;
        6  7  9 14 23];
...
%{
    ...
    table(Ntable,RowNames=["X=advertisement memory" "advertisement memory "],VariableNames="Y=Product purchase")
