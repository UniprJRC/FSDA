function [out , varargout]  = tclust_pcc(Y,k,alpha,restrfactor,varargin)
...
if cpc ~= 0
    if restrGPCM == true
        error('FSDA:tclust:WrongInputOpt', ...
            'Option ''cpc'' (common principal axes) is currently only supported when restrfactor is a scalar (not a GPCM struct).');
    end
    ...
    if startv1 ==true
        ...
        if usePriorSol==true && i==nselected
            ...
            for j=1:k
                ...
                % end
                % NOTE: the "if restrGPCM==false ... eig(...) ... end" block 
                % has been commented and replaced by the code below, 
                % marked by "added for common PC", executed once after this loop over j.
            end
            ...
            if restrGPCM == false
                ...
                % end of part added for common PC
        ...
        for j=1:k
            ...
            if mixt>=1
                if niini(j)>0
                    ...
                    % block removed: now computed once for all j
                    % if restrGPCM == false
                    %     % Eigenvalue eigenvector decomposition for group j
                    %     [Uj,Lambdaj] = eig(sigmaini(:,:,j));
                    % 
