...
%
%
%<a href="matlab: docsearchFS('Smulteda')">Link to the help function</a>
%
...
%
%         out.Loc  = length(bdp)-by-v  matrix containing S estimate of
%                   location for each value of bdp
%         out.Shape= v-by-v-by-length(bdp) 3D array  containing robust
...
for jj=1:length(bdp)
    ...
    if singsub==nsamp
        error('FSDA:Smulteda:NoFullRank','No subset had full rank. Please increase the number of subsets or check your design matrix X')
    end
    ...
    md = mahalFS(Y,superbestloc,superbestscale^2*superbestshape);
    ...
    Covar(:,:,jj)=superbestscale^2*superbestshape;
