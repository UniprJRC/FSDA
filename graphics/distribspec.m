...
elseif isstruct(pd)
    ...
    if isempty(userpdf) && isempty(usercdf)
        ...
        elseif parnum_c > 0 && parnum_p == -1
            ...
            switch parnum
                case 1
                    y = imag(usercdf(x+h*1i,parhat) / h);
                case 2
                    y = imag(usercdf(x+h*1i,parhat(1),parhat(2)) / h);
                case 3
                    y = imag(usercdf(x+h*1i,parhat(1),parhat(2),parhat(3)) / h);
            end

            % there are other ways to differentiate:
            %y0 = diff([eps ; usercdf(x,parhat)]) ./ diff([eps ; x]);
            %y1 = gradient(usercdf(x,parhat)) ./ gradient(x);
            %y2 = gradient(usercdf(x,parhat),x);
            % finite difference (f(x) - f(x-h)) / h;
            %y3 = (usercdf(x,parhat) - usercdf(x-h,parhat)) / h
            % central difference (f(x+h)-f(x-h))/(2*h);
            %y4 = (usercdf(x+h,parhat) - usercdf(x-h,parhat)) / (2*h);
