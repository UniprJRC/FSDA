...
%              plots.ColorMapLabelRows= character or cell which specifies
%                   whether the color of row labels must depend on a
%                   colormap. ColorMapLabelRows can be any of the
%                   VariableNames of out.OverviewRows. For example if
...
%                   plots.ColorMapLabelRows='' that is row labels have the
%                   same colors.
...
%              plots.ColorMapLabelCols= character or cell which specifies
%                   whether the color of column labels must depend on a
%                   colormap. ColorMapLabelCols can be any of the
%                   VariableNames of out.OverviewCols. For example if
...
if isstruct(plots)
    ...
    MarkerFaceColorColsSup=ColorColsSup;
    % Colormap for text labels of the row labels
    ColorMapLabelRows='';
...
if ~isempty(ColorMapLabelRows) && ~isempty(ColorMapLabelCols)
    error('FSDA:CorAnaplot:WrongInputOpt',['It is possible to specify a ' ...
...
if isempty(ColorMapLabelRows)
    ...
    if strcmp(ColorMapLabelRows,'CntrbDim2In')
        dist=out.OverviewRows.CntrbDim2In_1+out.OverviewRows.CntrbDim2In_2;
        disp('--------------------------------')
    ...
    elseif  ismember(ColorMapLabelRows,out.OverviewRows.Properties.VariableNames)
        ...
        disp('--------------------------------')
        disp(['The colormap of the row labels is proportional to ' ColorMapLabelRows])
    else
        error('FSDA:CorAnaplot:WrongInputOpt',"ColorMapLabelRows must be one of the VariableNmaes inside OverviewRows" + ...
            "or 'CntrbDim2In' or 'CntrbPnt2In'");
    end
    mindist=min(dist);
    dist1=(dist-mindist)/(max(dist)-mindist);
    ...
    mindist=min(dist1);
    maxdist=max(dist1);
    seqdist=mindist:((maxdist-mindist)/5):maxdist;
    colorlab=(num2str(max(dist)*seqdist',3));
...
if isempty(ColorMapLabelCols)
    ...
    if strcmp(ColorMapLabelCols,'CntrbDim2In')
        dist=out.OverviewCols.CntrbDim2In_1+out.OverviewCols.CntrbDim2In_2;
        disp('--------------------------------')
    ...
    elseif  ismember(ColorMapLabelCols,out.OverviewCols.Properties.VariableNames)
        ...
        disp('--------------------------------')
        disp(['The colormap of the Column labels is proportional to ' ColorMapLabelCols])
    else
        error('FSDA:CorAnaplot:WrongInputOpt',"ColorMapLabelCols must be one of the VariableNames inside OverviewCols" + ...
            "or 'CntrbDim2In' or 'CntrbPnt2In'");
    end
    mindist=min(dist);
    dist1=(dist-mindist)/(max(dist)-mindist);
