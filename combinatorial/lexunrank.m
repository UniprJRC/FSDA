...
%    N:  a non negative integer between 0 and bc(n,p)-1. 
%
%   IMPORTANT REMARK: 
...
% Output:
%
%   kcomb : row vector of length k with the k-combination.
...
% order(s) N.
% To generate all combinations in lexicographic order, it is convenient
% using the FSDA function combsFS(1:4,3). The MATLAB function with the same
% purpose, nchoosek(1:4,3), is much less efficient.
%
%
% Algorithmic details.
%
% Given the totally ordered set S={1,2,...,n), a k-combination is a subset
% {x_1, ..., x_k} of S. Consider the n-lists of elements of the set {0,1},
% i.e. the vertices of the hypercube V_n. Each k-combination {x_1,...,x_k}
% can be associated to the n-list having a 1 at position x_1, ..., x_k, and
% a 0 elsewhere. Example:
%   2-combinations of {1,2,3,4}:    {1,2} {1,3} {1,4} {2,3} {2,4} {3,4}
%   Corresponding 4-lists of {0,1}: 1100  1010  1001  0110  0101  0011
% The n-lists of {0,1) containing k times 1, and therefore equivalently the
% k-combinations of n-elements of S, can be generated in lexicographic
...
% However, a lexicographic algorithm implements a one-to-one correspondence
% between the k-combinations and the generation order, i.e. the set of
% numbers s = 1,...,(n!/((n–k)!k!)). This fact is used in our function to
% determine the n-list corresponding to the k-combination {x_1, ..., x_k}
% which would be generated by the lexicographic algorithm at a given
% desired position N. This is useful in a number of applications which
