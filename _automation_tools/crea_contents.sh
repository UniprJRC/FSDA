...
for fold in clustering combinatorial examples graphics multivariate regression utilities utilities_stat
do
	cd $fold
	>patrizia
...
cd utilities/privateFS
	
	>patrizia
	mv Contents.m patrizia
	echo "% privateFS" | tr '[:lower:]' '[:upper:]' >Contents.m
	echo "%" >>Contents.m
	echo "% Files :" >>Contents.m
	...
	echo "Contents.m in utilities/privateFS GENERATO !!!!"
	echo "DIFFERENZA  di Contents.m < nuovo_file > vecchio_file"
	sdiff Contents.m patrizia
	echo " <Enter> per continuare "
	read avanti
