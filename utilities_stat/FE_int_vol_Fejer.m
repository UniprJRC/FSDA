function ivar = FE_int_vol_Fejer(x,t, varargin)
...
%
% FE_int_vol_Fejer computes the integrated variance of univariate timeseries
% data from a diffusion process by the Fourier estimator with Fejer kernel
%
...
%
% See also: FE_int_vol.m, FE_spot_vol.m
%
% References:
%
...
%{
    ...
    % Analysis of the change of the integrated variance estimates of a univariate 
    % diffusion process as a function of the different values of the cutting frequency.
    n=1000;
    dt=1/n; t=0:dt:1;
    x=randn(n,1)*sqrt(dt); x=[0;cumsum(x)];
    cuttingfreq=(50:500)';
...
c_p=zeros(1,2*N+1); c_pp=zeros(1,2*N+1); term=zeros(1,2*N+1);

for k=1:(2*N+1)
    s=k-N-1;
    c_p(k)=sum(exp(-1i*const*s*t(1:end-1)).*r);
    c_pp(k)=sum(exp(1i*const*s*t(1:end-1)).*r);
    term(k)=(1-abs(s)/N)*c_p(k)*c_pp(k);
end
ivar=sum(term)/(N+1);
